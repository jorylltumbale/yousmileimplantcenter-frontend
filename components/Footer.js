...
const BlocksRow = styled(Row)`
  ...
  ${p => p.theme.media.minWidth('tablet-large')} {
    ${p => p.$format === 'locations' && (p.$gridSize === '4x' ? 'columns: 4' : 'columns: 3')}
  }  
`
...
const BlockCol = styled(Col)`
  ...
  ${p => p.theme.media.minWidth('tablet')} {    
    &:nth-last-child(-n+${p => p.$gridSize === '4x' ? 2 : 3}) {
      margin: 0;
    }
  ...
  ${p => p.theme.media.minWidth('desktop')} {
    &:nth-last-child(-n+${p => p.$gridSize === '4x' ? 4 : 3}) {
      margin: 0;
    }

    &:nth-child(${p => p.$gridSize === '4x' ? 4 : 3}n+1):not(:last-child) {
      display: flex;
      justify-content: flex-start;
    }

    &:nth-child(${p => p.$gridSize === '4x' ? 4 : 3}n+${p => p.$gridSize === '4x' ? 4 : 3}) {
      display: flex;
